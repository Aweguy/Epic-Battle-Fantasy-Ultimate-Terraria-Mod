...
namespace EpicBattleFantasyUltimate.Projectiles
{
	class JudiBeam : ModProjectile
	{
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			if (!player.channel)
			{
				...
				DrawLaser(spriteBatch, Main.projectileTexture[projectile.type], Main.player[projectile.owner].Center,
					projectile.velocity, 10, projectile.damage, -1.57f, 1f, 1000f, new Color(255, 255, 255, 255), (int)MOVE_DISTANCE);
			}
			return false;
		}
		...
		{
			//if(player.channel)
			for (Distance = MOVE_DISTANCE; Distance <= MaxDistance; Distance += 5f)
			{
		...
		private void ChargeLaser(Player player)
		{
			...
			//{
				...
				{
					Charge++;
					MaxDistance = 10 * Charge;
					projectile.damage = (int)(100 * Charge);
				}
