    ...
    public class RubyOre : ModNPC
    {
        ...
        public override void OnHitPlayer(Player target, int damage, bool crit)
        {
            ...
            if (npc.life >= npc.lifeMax * 0.40)
            {
                ...
                {
                    Projectile.NewProjectile(npc.Center.X, npc.Center.Y, 0f, 0f, ModContent.ProjectileType<RubyExplosion>(), 60, 5f, Main.myPlayer, 0, 1);
        ...
        private void Movement(NPC npc, Player player)
        {
            ...
            if (!Dashing && State != OreState.Stunned)//This boolean shows when the ore is actually dashing. We check if it's not true so it only chases the player while not dashing
            {
                ...
                MoveSpeedMult += num1276 / 95f; //Balancing the speed. Lowering the division value makes it have more sharp turns.
                int MoveSpeedBal = 95; //npc does the same as the above.... I do not understand.
