	...
	public class SapphireOre : ModNPC
	{
		...
		private void Movement(NPC npc, Player player)
		{
			...
			if (!Dashing && State != OreState.Stunned)//This boolean shows when the ore is actually dashing. We check if it's not true so it only chases the player while not dashing
			{
				...
				float MoveSpeedMult = 9f; //How fast it moves and turns. A multiplier maybe?
			...
			{
				if ((Vector2.Distance(player.Center, npc.Center) <= 16 * 25f && AttackTimer < 120))//Range control for the charging.
				{
					AttackTimer++;
				...
				{
					npc.velocity = Vector2.Normalize(player.Center - npc.Center) * 13f;//the speed that the ore will dash towards the player
