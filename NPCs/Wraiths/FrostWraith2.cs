...
namespace EpicBattleFantasyUltimate.NPCs.Wraiths
{
	public class FrostWraith2 : ModNPC
	{
		...
		public override void AI()
		{
			...
			Shooting(player);//Spike shooting
		...
		{
			if (Main.hardMode && spawnInfo.Player.ZoneSnow && NPC.downedPlantBoss)
			{
				return 0.02f;
			}
		...
		{
			npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Wool>(), 1, 2, 2));
			npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<SilkScrap>(), 5, 1, 3));
			npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<SolidWater>(), 2, 1, 5));
		}
		#endregion NPCLoot
	}
}